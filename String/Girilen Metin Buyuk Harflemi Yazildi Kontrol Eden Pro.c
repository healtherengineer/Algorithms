...
int main ()
{
	...
	for (i = 0 ; Cumle[i] != '\0' ; i++)
	{
		...
		for (j = 0 ; Kontrol[j] != '\0' ; j++)
		{
			...
			{
				c = 1; // buyuk harf bulduðunda 1 oluyor ki o yazý buyuk harfle yazýlmýþmý yazýlmamýsmý kontrol ediyor
			}	
		}
		if (c == 1)
			{
				s = s + 1; //eger s cumle boyutuna esitse tamamen buyuk harfle yazýlmýstýr diyor
			}
	}
